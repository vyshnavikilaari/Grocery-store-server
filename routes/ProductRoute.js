...
router.post('/add', async (req, res) => {
    try {
        ...
        const { title, img, price,offer } = newproduct
        if (!title || !img || !price || !offer) {
...
router.delete('/delete/:id', async (req, res) => {
    try {
        ...
        const existingproduct = await Products.findOne({ _title: title })
        if (!existingproduct) {
            res.status(404).json({ message: "Product not found" })
        }
        await Products.findByIdAndDelete(title)
